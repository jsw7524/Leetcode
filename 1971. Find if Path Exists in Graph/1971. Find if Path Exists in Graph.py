...
class Solution:
    ...
    def BFS(self, x, target):
        queue=[x]
        while len(queue) > 0:
            currentNode=queue.pop(0)
            if 0==self.visit[currentNode]:
                self.visit[currentNode]=1
                if target == currentNode:
                    return True
                queue+=self.graph[currentNode]
        return False

    def validPath(self, n: int, edges: List[List[int]], start: int, end: int) -> bool:
        self.visit=[0]*n
        self.graph={ v:[] for v in range(n)}
        for v1,v2 in edges:
...
sln=Solution()
assert True==sln.validPath(n = 3, edges = [[0,1],[1,2],[2,0]], start = 0, end = 2)
assert False==sln.validPath(n = 6, edges = [[0,1],[0,2],[3,5],[5,4],[4,3]], start = 0, end = 5)
assert False==sln.validPath(n = 100, edges = 
